...
export async function servicesRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  // owns the preHandler so request.supabase is populated for every route.
  await fastify.register(async (s) => {
    ...
    s.get("/api/services/:id", async (request, reply) => {
    ...
    s.post("/api/services/:id/restart", async (request, reply) => {
      const { id } = request.params as { id: string };
