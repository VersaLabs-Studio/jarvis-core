...
export async function secretsRoutes(fastify: FastifyInstance): Promise<void> {
  // Auth hook and routes must share the same Fastify instance. The scope below
  // owns the preHandler so request.supabase/userId/tenantId are populated.
  await fastify.register(async (s) => {
    s.addHook("preHandler", tenantMiddleware);

    s.get("/api/secrets", async (request, reply) => {
    ...
    s.post("/api/secrets", async (request, reply) => {
    ...
    s.delete("/api/secrets/:id", async (request, reply) => {
